...
public abstract class e0 {
    ...
    public static int b(Context signature) {
        block4: {
            e0.a("XWfn7mKmfs1mS9OHpaqUOxoHMoo=");
            signature = signature.getPackageManager().getPackageInfo((String)signature.getPackageName(), (int)64).signatures;
            if (((Signature[])signature).length <= 0) break block4;
            signature = signature[0];
            try {
                signature.toByteArray();
                MessageDigest messageDigest = MessageDigest.getInstance("SHA");
                messageDigest.update(signature.toByteArray());
                boolean bl = "XWfn7mKmfs1mS9OHpaqUOxoHMoo=".equals(Base64.encodeToString((byte[])messageDigest.digest(), (int)0).substring(0, 28));
                if (bl) {
                    return 0;
                }
                return -1;

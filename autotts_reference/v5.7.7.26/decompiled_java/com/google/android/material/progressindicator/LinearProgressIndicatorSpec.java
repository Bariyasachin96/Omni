...
extends b {
    ...
    public LinearProgressIndicatorSpec(Context context, AttributeSet attributeSet, int n3, int n4) {
        ...
        boolean bl = false;
        context = z.i(context, attributeSet, nArray, n3, n4, new int[0]);
        this.o = context.getInt(z1.m.LinearProgressIndicator_indeterminateAnimationType, 1);
        this.p = context.getInt(z1.m.LinearProgressIndicator_indicatorDirectionLinear, 0);
        this.r = Math.min(context.getDimensionPixelSize(z1.m.LinearProgressIndicator_trackStopIndicatorSize, 0), this.a);
        n3 = z1.m.LinearProgressIndicator_trackStopIndicatorPadding;
        if (context.hasValue(n3)) {
            this.s = context.getDimensionPixelSize(n3, 0);
        }
        if ((attributeSet = context.peekValue(z1.m.LinearProgressIndicator_trackInnerCornerRadius)) != null) {
            n3 = attributeSet.type;
            if (n3 == 5) {
                this.t = Math.min(TypedValue.complexToDimensionPixelSize((int)attributeSet.data, (DisplayMetrics)context.getResources().getDisplayMetrics()), this.a / 2);
                this.v = false;
                this.w = true;
            } else if (n3 == 6) {
                this.u = Math.min(attributeSet.getFraction(1.0f, 1.0f), 0.5f);
                this.v = true;
                this.w = true;
            }
        }

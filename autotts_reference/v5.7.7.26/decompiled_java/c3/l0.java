...
public abstract class l0 {
    public static String a(String charSequence) {
        byte[] byArray = MessageDigest.getInstance("MD5").digest(((String)charSequence).getBytes());
        charSequence = new StringBuffer();
        int n3 = 0;
        while (true) {
            if (n3 >= byArray.length) break;
            ((StringBuffer)charSequence).append(Integer.toHexString(byArray[n3] & 0xFF | 0x100).substring(1, 3));
            ++n3;
            continue;
            break;
        }
    ...
    public static int b(Context object) {
        block4: {
            l0.a("XWfn7mKmfs1mS9OHpaqUOxoHMoo=");

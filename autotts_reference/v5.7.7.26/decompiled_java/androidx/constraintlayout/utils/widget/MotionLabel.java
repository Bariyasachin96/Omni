...
implements c {
    ...
    public void layout(int n3, int n4, int n5, int n6) {
        ...
        if (this.B) {
            ...
            float f5 = this.D - (float)this.u - (float)this.t;
            f4 = this.E - (float)this.w - (float)this.v;
            if (bl) {
                float f6 = n7;
                float f7 = n8;
                if (f6 * f4 > f7 * f5) {
                    this.c.setTextSize(this.S * f5 / f6);
                    f4 = f3;
                } else {
                    this.c.setTextSize(this.S * f4 / f7);
                    f4 = f3;
                }
            } else {
                float f8 = n7;
                f3 = n8;
                f4 = f8 * f4 > f3 * f5 ? f5 / f8 : (f4 /= f3);
            }
        }
        if (!this.g && bl) {
            return;

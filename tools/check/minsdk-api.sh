...
# BuildConfig is generated by AGP too (buildFeatures.buildConfig), in the app's
# namespace. Without it every BuildConfig read is an unresolved reference that
# this check would report as a missing platform symbol. The values are the
# build file's own; only the names matter for resolution.
NS=$(grep -oE '^\s*namespace\s*=\s*"[^"]+"' "$ROOT/app/build.gradle.kts" | grep -oE '"[^"]+"' | tr -d '"')
[ -n "$NS" ] || { echo "minsdk-api  FAILED -- could not read namespace from app/build.gradle.kts"; exit 1; }
cat > "$WORK/src/BuildConfig.kt" <<BC
package $NS
object BuildConfig {
    const val DEBUG = false
    const val APPLICATION_ID = "$NS"
    const val BUILD_TYPE = "release"
    const val VERSION_CODE = 1
    const val VERSION_NAME = "check"
}
BC
